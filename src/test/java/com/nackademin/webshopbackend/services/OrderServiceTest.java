...
class OrderServiceTest {
	...
	@BeforeEach
	public void init() {
		this.customer = Optional.of(new Users(1L, "email@email.se",
				"username", "pass", "firstName", "lastName",
				"number", "USER", new String[1],
				new Address(), true, true, LocalDateTime.now(), LocalDateTime.now()));
		this.order = new Orders(1L, customer.get(), LocalDate.now().toString(),
				PENDING, 0.0, LocalDateTime.now(), LocalDateTime.now());
	...
	class setOrderStatusToPaidTest {
		...
		void unsuccessfullyTest() {
			assertThrows(NotFoundException.class, () -> orderService.setOrderStatusToPaid(order.getId()));
		}
	}
}